...
class Dataset:
    ...
    def trainLoader(self):
        train_file_names = sorted([line.rstrip() for line in open(os.path.join(self.data_root,'meta_data/scannetv2_train.txt'))])
        train_file_names = list(map(lambda name: os.path.join(self.data_root,self.dataset,'{}_pointgroup.pth'.format(name)),train_file_names))
    ...
    def valLoader(self):
        val_file_names = sorted([line.rstrip() for line in open(os.path.join(self.data_root,'meta_data/scannetv2_val.txt'))])
        val_file_names = list(map(lambda name: os.path.join(self.data_root,self.dataset,'{}_pointgroup.pth'.format(name)),val_file_names))
